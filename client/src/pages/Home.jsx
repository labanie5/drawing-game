...
export default function Home({ onJoin }) {
  ...
  useSocketEvent('room-created', ({ code, players, rounds }) => {
    setLoading(false);
    onJoin({ socket, code, players, rounds, myId: socket.id, isHost: true });
  });

  useSocketEvent('room-joined', ({ code, players, rounds }) => {
    setLoading(false);
    onJoin({ socket, code, players, rounds, myId: socket.id, isHost: false });
  });
  ...
  function handleCreate(e) {
    ...
    setError('');
    setLoading(true);
    socket.emit('create-room', { name: name.trim(), rounds });
  }
      ...
      {view === 'create' && (
        <form className="home-form" onSubmit={handleCreate}>
          ...
          <div className="form-actions">
            <button type="button" className="btn btn-ghost" onClick={() => { setView('main'); setError(''); }}>
              Back
            </button>
